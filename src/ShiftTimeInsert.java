...
public class ShiftTimeInsert extends JFrame implements ActionListener,ItemListener,FocusListener{
	JLabel l1,l2,l3,l4;
	JButton bt1,bt2;
	JTextField t1,t2,t3;
	JComboBox c1;
	...
	void filldata() {
		...
		try {
			...
			String sql="select SHIFTMASTER_ID from Shift_master";
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==bt1) {
			...
			obj.makeConnection();
			String str=obj.STInsert(a, b, c, d);
	...
	public void itemStateChanged(ItemEvent ae) {
		...
		if(ae.getSource()==c1) {
			...
			try {
				...
				String sql="select SHIFT_MASTER.MON,SHIFT_MASTER.TUE,SHIFT_MASTER.WED,SHIFT_MASTER.THU,SHIFT_MASTER.FRI,SHIFT_MASTER.SAT,SHIFT_MASTER.SUN,SHIFT_TIME.SHIFTTIME_ID,SHIFT_TIME.SHIFTTFROM,SHIFT_TIME.SHIFTTTO from shift_time,shift_master where shift_time.shiftmaster_id=shift_master.SHIFTMASTER_ID and SHIFT_MASTER.SHIFTMASTER_ID=?";
				PreparedStatement st=conn.prepareStatement(sql);
				st.setString(1,a);
				ResultSet rs=st.executeQuery();
				while(rs.next()) {
				ta1.setText("Shift from Monday to Sunday"+"\n"+"Mon->"+rs.getString(1)+"\t"+"Tues->"+rs.getString(2)+"\t"+"Wed->"+rs.getString(3)+"\t"+"Thurs->"+rs.getString(4)+"\t"+"Fri->"+rs.getString(5)+"\t"+"Sat->"+rs.getString(6)+"\t"+"Sun->"+rs.getString(7)+"\n"+"Shift time->"+rs.getString(8)+"\t"+"Shift From->"+rs.getString(9)+"\t"+"Shift To->"+rs.getString(10));
				}
				conn.close();
	...
	public void focusLost(FocusEvent e) {
		...
		if(e.getSource()==t1) {
			...
			if(st.charAt(0)!='S'||st.charAt(1)!='T'||st.length()!=6) {
				JOptionPane.showMessageDialog(this,"ID's first letter S, Second T and length = 6");
				t1.grabFocus();
			}
		}
	}
